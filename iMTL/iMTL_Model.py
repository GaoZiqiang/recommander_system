...
#----------------------------------------------
# Temporal-aware activity encoder
...
#----------------------------------------------
# Spatial-aware location preference encoder
# right x inputs: poi prediction - poi, dist

input_x_dist = tf.nn.embedding_lookup(dist_emb, x_dist) # [batch, seq_len, dim] # the embedding of distance
input_x_poi = tf.nn.embedding_lookup(poi_emb, x_poi) # [batch, seq_len, dim] # the embedding of POI

inputs_x_r =  tf.matmul(input_x_dist, tf.expand_dims(W_dist,0)) + tf.matmul(input_x_poi, tf.expand_dims(W_poi,0)) # [batch, seq_len, dim]



# y inputs ??? y inputs 是什么？
...
#----------------------------------------------
    ...
    # left loss: category and type prediction。 category是POI的category
